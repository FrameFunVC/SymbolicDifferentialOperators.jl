...
is_first_dimension_name(d::AbstractDiffOperator, x::Char, dns::NTuple{N,Char}=dimension_names(d)) where N =
    x == dns[1]
full_element(::IdentityDifferentialOperator, dim::NTuple{N,Char}) where N =
    ProductDifferentialOperator{dim}(ntuple(k->0,Val(N)))
...
full_element(::ZeroDifferentialOperator, dim::NTuple{N,Char}) where N =
    0*ProductDifferentialOperator{dim}(ntuple(k->0,Val(N)))

struct LaplaceOperator <: AbstractDiffOperator
end

export Δ
Δ = LaplaceOperator()

order(::LaplaceOperator) = 2

string(::LaplaceOperator) = "Δ"

full_element(::LaplaceOperator, dim::NTuple{N,Char}) where N =
    +((ProductDifferentialOperator{dim}(ntuple(k->(k==i) ? 2 : 0,Val(N))) for (i,c) in enumerate(dim))...)


struct NormalDerivativeOperator{D} <: AbstractDiffOperator
    domain::D
end
normal(x,op::NormalDerivativeOperator) = normal(x,op.domain)

export Δ
δᵀ = NormalDerivativeOperator

order(::NormalDerivativeOperator) = 1

string(::NormalDerivativeOperator) = "δᵀ"

full_element(::NormalDerivativeOperator, dim::NTuple{N,Char}) where N =
    error("I do not know what to do here.")
...
full_element(s::ScaledDifferentialOperator, dim::NTuple{N,Char}) where N =
    coefficient(s)*full_element(operator(s), dim)
...
function full_element(p::PartialDifferentialOperator, dim::NTuple{N,Char}) where N
    @assert dimension_name(p)∈dim
    ProductDifferentialOperator{dim}(tuple((get(dictionary(p), c, 0) for c in dim)...))
end
...
function full_element(p::ProductDifferentialOperator, dim::NTuple{N,Char}) where N
    for c in dimension_names(p)
        @assert c ∈ dim
    end
    ProductDifferentialOperator{dim}(tuple((get(dictionary(p), c, 0) for c in dim)...))
end
...
full_element(s::SumDifferentialOperator, dim::NTuple{N,Char}) where N =
    SumDifferentialOperator(full_elements(s, dim)...)

full_elements(s::SumDifferentialOperator, dim=dimension_names(s)) = [full_element(op, dim) for op in elements(s)]
